        ...
        if (!empty($borrow_arr)) {
            if ($val['type'] != 4) {
                foreach ($borrow_arr as $key => $val) {
                    $position_stock_arr = Db::name('stock_position')->where('sub_id', $val['stock_subaccount_id'])->field('gupiao_code,sub_id,stock_count')->select();
                    $code               = '';
                    $temarr             = array();
                    foreach ($position_stock_arr as $k => $v) {
                        $code                      = $code . $v["gupiao_code"] . ',';
                        $temarr[$v["gupiao_code"]] = $v['stock_count'];
                    }
                    ...
                    if (!empty($info)) {
                        foreach ($info as $ke => $value) {
                            $current_price = $value['current_price'];                //股票实时价格
                            $market_val += $temarr[$value['code']] * $current_price; //最新市值
                        }
                    }
                    if (!empty($market_val)) {
            ...
            return json(['data' => $nums, 'code' => 1, 'message' => '需要增加保证金的数量']);
        } else {
